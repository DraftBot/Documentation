...
::hint{ type="danger" }
  **DraftBot**, peut sanctionner avec une note un membre uniquement si vous disposez de la permission **`"Gérer les messages"`** ou que l'un de vos rôles peut utiliser la commande \</note> sur votre serveur.
...
Le fait d'expulser un membre fera quitter le membre du serveur, mais il pourra toujours revenir avec une autre invitation. Si vous souhaitez qu'il ne puisse pas revenir, consultez le `bannissement` dans le sommaire.
...
Le fait de **bannir un membre fera quitter le membre du serveur et il ne pourra jamais y revenir**. Vous pouvez également bannir temporairement le membre en question. Si vous souhaitez qu'il puisse revenir sur votre serveur, consultez l'`expulsion` dans le sommaire.
...
![Historique de sanctions d'un membre](../assets/moderation/cmd_config/view_all_punishments.png)
...
::tabs
  ...
  ::tab{ label="Via le panel" }
    ...
    - **Masquer le nom du modérateur en MP ➜** Le nom du modérateur ne sera pas visible par le membre dans le message reçu en message privés.
    - **Masquer les réponses des commandes ➜** Lorsqu'un modérateur effectuera une commande de modération *(voir les commandes ci-dessus)*, le message indiquant cette sanction sera uniquement visible par le modérateur et non par l'ensemble des membres.
...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    ::hint{ type="info" }
      Vous auvez également la possibilitée de retirer toutes les sanctions prédéfinies en cliquant sur le bouton **`Réinitialiser`**.
    ::
    ::hint{ type="warning" }
