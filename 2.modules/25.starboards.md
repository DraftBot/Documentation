...
#### Salon pouvant être mis en avant
  ...
  - Accorde la possibilité de choisir **un salon limitant les messages pouvant apparaitre dans le starboard à se salon *(ou plusieurs autres)*** et inversement.
...
#### Rôles pouvant être mis en avant
  ...
  - Grâce à cette option, vous pouvez **définir uniquement les messages d'un groupe d'utilisateur *(via des rôles)* puissent apparaitre dans le starboard** et inversement.

#### Rôles pouvant voter

  - Vous pouvez définir, via cette option, **un ou des rôles limitant ainsi la posiblité de voter pour qu'un message se retrouve dans le starboard** et inversement.

### Personnalisation

  - Avec ces options vous permet de **modifier le contenu du message**. Le **mettre ou non dans un embed** et aussi de **changer sa couleur**.
  
  ::hint{ type="info" }
    Changer la couleur : on parle ici de changer la couleur de l'embed donc il faut activier l'option pour avoir celle-ci. *(Avantage [**premium** ✨](https://www.draftbot.fr/premium/))*
  ::
