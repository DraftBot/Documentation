...
Lorsque vous créerez un salon vocal temporaire, vous aurez la possibilité de configurer votre salon par le biais d'un [message](/privateroom#utilisation-du-configurateur) *(option désactivable)*.
...
- **Ouvert** : Cette fonction permet d'ouvrir le salon à tous les membres, excepté ceux de la **liste noire** et des dérogations par défaut.
  ...
  ::hint{ type="info" }
    En mode ouvert, les utilisateurs 'whitelist' ont les permissions micro, vidéo et soundboards.
  ::
...
- **Liste Blanche** : Cette fonction permet d'ajouter des membres qui seront **autorisés à rejoindre** le salon peut importe le mode (Public, Fermé, Privé) et recevront toutes les permissions micro, vidéo & soundboards.
...
![Comment utiliser les variables](/.assets/privateroom/view_rename_panel.png)
