...
## Utilisation de embed de réglages
...
::tabs
  ...
  ::tab{ label="Depuis le panel" }
    Allez dans le panel, puis dans la catégorie [Communautaire](www.draftbot.fr/dashboard/first/community). Ensuite, cliquez sur `Salons vocaux temporaires (privateroom)` et enfin sur le bouton `Configuration du salon`.
