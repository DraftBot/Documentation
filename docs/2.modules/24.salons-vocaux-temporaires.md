...
contributors: ['tellvex', 'kazotka', 'theorik', 'elvhira']
...
::collapse{ label="Liste détaillée de chaque fonction de l'embed de configuration" }
  ...
  ::hint{ type="info" }
    En mode ouvert, les utilisateurs présents sur la liste blanche ont les permissions micro, vidéo et soundboards.
  ::

  - **Fermé** : Cette fonction verrouille le salon pour tous les membres, exceptés ceux de la **liste blanche** et les dérogations par défaut.

  - **Liste Blanche** : Cette fonction permet d'ajouter des membres qui seront **autorisés à rejoindre** le salon peu importe le mode (Public, Fermé, Privé) et qui recevront toutes les permissions micro, vidéo & soundboards.
  ...
  - **Vidéo** : Tout comme la fonction précédente, elle permet d'autoriser/refuser la permission d'utiliser la caméra ou le partage d'écran, à tous les participants du salon vocal.
...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    Pour activer les salons vocaux temporaires, rendez-vous dans le menu **Salons vocaux temporaires** de la commande \</config> et cliquez sur le bouton **Créer un hub**.
    ...
    **DraftBot** vous demandera ensuite de choisir **la catégorie** où seront créés les salons vocaux.
    ...
    Pour activer ou désactiver la création de salons temporaires, cliquez sur le bouton **Activer le système**. Vous verrez alors le menu se mettre à jour.

    ### Changer la catégorie des Salons Vocaux Temporaires

    Pour changer la catégorie des salons vocaux temporaires, cliquez sur le bouton **Catégorie**. Vous devrez ensuite choisir de créer une nouvelle catégorie ou en sélectionner une existante.

    ### Changer le salon hub

    Pour changer le salon vocal de redirection vers les salons temporaires, cliquez sur le bouton **Hub (salon)**. Vous devrez ensuite indiquer [l'identifiant du salon](/docs/autres/recuperer-un-identifiant) de votre choix.

    ### Permissions par défaut des salons temporaires

    Pour définir les permissions par défaut des salons temporaires, cliquez sur le bouton **Permissions par défaut**. Vous pourrez alors choisir entre les permissions du **salon vocal hub** ou les **permissions de la catégorie**.
    ...
    Vous pouvez définir la limite de membres autorisés à rejoindre un salon vocal en appuyant sur le bouton **Limite de membre**. Vous devrez ensuite indiquer une valeur entre 0 et 99 qui correspond au nombre maximum de participants. Indiquer "0" comme limite revient à la désactiver.

    Par défaut, la limite de membres est définie à **99 participants**.

    ::hint{ type="warning" }
      Veuillez également noter que la limite de membres peut être changée à tout moment par **le créateur du salon** s'il possède la permission **Modifier le salon**.
    ::

    ### Ajouter/Supprimer un salon permanent

    ::hint{ type="info" }
      Par défaut, lorsque le dernier membre quitte un salon vocal de la catégorie des salons vocaux temporaires, le salon temporaire est instantanément supprimé.
    ::

    Pour qu'un salon soit conservé dans la catégorie même après déconnexion du dernier membre, appuyez sur le bouton **Salons permanents**. Un menu s'ouvrira alors et vous permettra d'ajouter ou supprimer des salons vocaux qui seront marqués comme permanents.

    ### Activer/Désactiver l'embed de réglages

    Pour activer ou désactiver l'embed de réglages,utilisez le bouton.
    ...
    Pour personnaliser les boutons de réglages, appuyez tout d'abord sur le bouton **Personnaliser les boutons de réglages**. Vous verrez alors un sélecteur qui vous permettra de cocher toutes les options que vous souhaitez rendre disponibles dans l'embed de configuration du salon.
    ...
    Pour ajouter/supprimer des rôles qui auront accès à l'embed de configuration, appuyez sur le bouton **Rôles d'accès aux réglages**. Cela vous amènera sur un message composé de 4 boutons (pouvant varier selon le type de liste de rôles choisi).
    ...
    ::collapse{ label="Restriction de rôles" }
      - **Ajouter des rôles interdits** : Permet d'ajouter des rôles à la liste des rôles interdits d'utilisation de l'embed de configuration.
          ...
          - **Restreindre à des rôles** : Ce bouton change le type de filtrage, permettant de marquer les rôles ajoutés à la liste comme rôles autorisés à l'utilisation de l'embed de configuration du salon.
    ::
    ...
    Pour supprimer un hub de salons vocaux temporaires, allez dans le menu **Salons vocaux temporaires** de la commande \</config> et cliquez sur le bouton **Supprimer un hub**.
  ...
  ::tab{ label="Via le panel" }
    ...
    Pour créer un hub, cliquez sur le bouton "**Créer un hub**". Il y aura une liste d'informations à remplir, qui seront mentionnées dans la partie **Modifier un Hub**.
    ...
    Vous pouvez attribuer certaine(s) permission(s) supplémentaire(s) au propriétaire du salon. Pour cela, **cochez les permissions** que vous voulez attribuer.
    ...
    Pour ajouter des salons permanents, appuyez sur l'icône "**+**" et indiquez le nom du salon.

    Pour supprimer un salon permanent, **appuyez sur la croix** à côté du salon permanent à retirer.

    ### Définir la limite de membres

    Pour définir la limite de membres, sélectionnez la valeur que vous voulez. Notez également que la valeur "**illimité**" retire la limite de membres.
    ...
    - En retirant l'accès par défaut à tous les rôles et en ajoutant les rôles exemptés.
    - En donnant l'accès par défaut à tous les rôles sauf ceux listés.

    Pour cela, appuyez sur "**Avec**" pour la première option, ou bien "**Sans**" pour la deuxième option.

    Pour ajouter des rôles, cliquez sur le bouton "**+**". Pour supprimer des rôles, cliquez sur la **croix** à côté de leur nom.

    ### Définir les boutons de configuration
    ...
    Pour supprimer un hub, rendez-vous dans la partie **Salons vocaux privés** du [panel de DraftBot](/dashboard/user/profil) et **supprimez** le hub de votre choix.
