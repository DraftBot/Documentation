...
::tabs
  ::tab{ label="Depuis la commande /config" }
    Pour modifier l'emplacement des salons vocaux lors de la création, il vous suffira d'exécuter la commande \</config>, de sélectionner depuis le menu déroulant "`Salons vocaux temporaires`" et de cliquer sur le bouton "`Hub (salon)`".
