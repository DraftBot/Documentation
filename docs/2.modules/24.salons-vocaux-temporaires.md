...
::hint{ type="info" }
  Après chaque changement, **DraftBot** vous confirme la nouvelle permission dans un message visible par vous seul. Ce message y liste les membres qui n'y sont pas soumis, de par le biais d'une permission leur étant accordée (ou refusée) ailleurs par un rôle, par la permission **Administrateur** ou par une dérogation du salon. Pour le **Micro**, il rappelle aussi que les membres déjà connectés devront se reconnecter, Discord ne permettant pas de faire autrement.
::
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécuter la commande \</config>, puis sélectionner depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Rôles intouchables`.
