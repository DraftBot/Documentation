...
::tabs  
  ...
  ::tab{ label="Inventaire / Économie" }
    ...
    *En utilisant cette commande, le membre achètera l'objet sélectionné.*
    ::collapse{ label="Afficher un exemple :" }
    Voici une commande qui permet à son utilisateur d'acheter l'item "🦊 Renard en peluche" :

      ![Exemple d'action Ajouter des rôles](../assets/commandes-personnalisees/ex_cmd_perso_role.png)
      ::hint{ type="warning" }
        À l'exécution, aucune confirmation n'est demandée au membre, et aucun message ne s'affiche par défaut *(sauf en cas d'erreur)* :\
        N'hésitez pas à utiliser cette action combinée avec une action "**Envoyer un message**" !
      ::
    ::
