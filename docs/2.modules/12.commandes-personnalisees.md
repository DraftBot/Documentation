...
::tabs  
  ::tab{ label="Message" }
    ...
    *Lors de l'exécution de la commande, DraftBot enverra un message.* 
    ...
    ::
    #### Exemple :
    Voici un exemple d'action **Envoyer un message**, réalisée depuis le [Panel](/dashboard/first/custom-commands) :
    ![Exemple d'une commande personnalisée réalisée depuis le Panel](../assets/commandes-personnalisees/ex_cmd_perso_txt.png)
    Lorsque la commande est appelée sur un serveur, on obtient ceci :
