...
updated_at: '2025-02-28 16:28:00'
---
...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    * Les rôles ou salons [interdits/autorisés](https://draftbot.fr/docs/modules/commandes-personnalisees#options-avancees),
    * Si la commande doit [apparaître](https://draftbot.fr/docs/modules/commandes-personnalisees#options-avancees) dans le \</aide> ou non,
    * Si vous souhaitez ajouter un [mode lent](https://draftbot.fr/docs/modules/commandes-personnalisees#options-avancees) à votre commande,
    * Si vous souhaitez définir une [condition](https://draftbot.fr/docs/modules/commandes-personnalisees#options-avancees) pour l'utiliser.
  ...
  ::tab{ label="Depuis le panel" }
    ...
    ::hint{ type="success" }
      Pour plus de personnalisation, déroulez le [menu "**avancé**"](https://draftbot.fr/docs/modules/commandes-personnalisees#options-avancees) !
    ::
    ::hint{ type="info" }
  ...
  ::tab{ label="Rôles" }
    ...
    *Lors de l'exécution de la commande, Draftbot retirera le/les rôles sélectionnés.*
    ...
    ::collapse{ label="Afficher un exemple :" }
      ...
      ::hint{ type="success" }
        Si j'écris sur Discord `!retrograde-a @Drafman`, le membre \<@Draftman> perd instantanément le rôle \<@A> !
      ::
    ::
    ::hint{ type="info" }
  ...
  ::tab{ label="Inventaire / Économie" }
    ...
    *En utilisant cette commande, le membre achètera l'objet sélectionné.*
    ::collapse{ label="Afficher un exemple :" }
      ...
      ::
      ::hint{ type="info" }
        L'achat est réalisé aussitôt la commande effectuée, aucune confirmation préalable n'est demandée, et aucune confirmation d'achat n'est envoyée :\
      💡Pensez à combiner cette action avec l'action **Envoyer un message** !
      ::
    ::
    ...
    *Lors de l'exécution de la commande, DraftBot ajoutera un montant défini d'argent au membre.*
    ::collapse{ label="Afficher un exemple :" }
      ...
      ::hint{ type="success" }
        Si j'écris sur Discord `!donation @Drafman`, le membre \<@Draftman> reçoit instantanément 10💰 !
      ::
      ::hint{ type="info" }
        Vous pouvez aussi décider que cette commande ajoute la somme au membre qui l'utilise, et non à un membre mentionné. Il suffit de décocher la case "cibler un autre membre".
      ::
    ::
