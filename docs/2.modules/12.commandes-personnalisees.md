...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    * Si vous souhaitez ajouter un mode lent à votre commande :
      * Nombre d'utilisations d'affilée par le même utilisateur
      * Durée de rechargement après avoir atteint ce nombre
    * Si vous souhaitez ajouter à votre commande une condition parmi :
      * Niveau requis
      * Argent requis (vous pouvez choisir si la somme définie doit-être payée)
      * Objet d'inventaire requis (vous pouvez choisir si l'objet doit être retiré au membre)
  ...
  ::tab{ label="Depuis le panel" }
    ...
    Remplissez les divers champs (nom, description, actions...) puis cliquez sur le bouton "Créer" en bas à droite de votre commande personnalisée.

    ![Panel de configuration des commandes personnalisées](../assets/commandes-personnalisees/view.png){ width=600px }
    ...
    Pas de problème ! Cliquez sur "Modifier" ou "Supprimer" sur le côté droit. ![Boutons de modification ou suppression d'une commande personnalisée](../assets/commandes-personnalisees/edit-command.png)
    ::
  ::
::
