...
::tabs  
  ...
  ::tab{ label="Inventaire / Économie" }
    ...
    *En utilisant cette commande, le membre achètera l'objet sélectionné.*
    ::collapse{ label="Afficher un exemple :" }
      ![Exemple d'action Article de la boutique](../assets/commandes-personnalisees/ex_cmd_perso_role.png)
    ::

    2. **Ajouter de l'argent** \
    *Lors de l'exécution de la commande, DraftBot ajoutera un montant défini d'argent au membre.*
    ::collapse{ label="Afficher un exemple :" }
      ![Exemple d'action Ajouter de l'argent](../assets/commandes-personnalisees/ex_cmd_perso_role.png)
    ::

    ::hint{ type="info" }
      Pour la commande **Ajouter de l'argent**, vous pouvez décider si la modification doit s'appliquer à l'utilisateur de la commande, ou à une autre personne, mentionnée dans la commande.
    ::
  ::
::
