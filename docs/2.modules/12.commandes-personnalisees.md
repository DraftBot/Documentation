  ...
  ::tab{ label="Inventaire / Économie" }
    ...
    *Lors de l'exécution de la commande, DraftBot ajoutera un montant défini d'argent au membre.*
    ::collapse{ label="Afficher un exemple :" }
      ...
      ::
      ::hint{ type="info" }
        Vous pouvez aussi décider que cette commande ajoute la somme au membre qui l'utilise, et non à un membre mentionné. Pour ce faire, décochez la case "cibler un autre membre".
      ::
    ::
