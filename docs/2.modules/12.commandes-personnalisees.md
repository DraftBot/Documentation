...
::tabs
  ...
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/custom-commands){target=_blank}
    ...
    ![Panel de configuration des commandes personnalisées](../assets/commandes-personnalisees/panel_customcommands_0.png)
    ...
    ::hint{ type="info" }
      ...
      ![Boutons de modification ou suppression d'une commande personnalisée](../assets/commandes-personnalisees/edit_command_0.png)
    ::
  ::
::
...
::tabs
  ::tab{ label="Message" }
    ...
    *Lors de l'exécution de la commande, DraftBot enverra un message.*\
    ::hint{ type="info" }
      Le message peut contenir du markdown et des variables. Si vous le configurez depuis le [Panel](/dashboard/first/custom-commands){target=_blank}, vous pouvez même y ajouter des embeds !
    ::
    ::collapse{ label="Afficher un exemple :" }
      Voici une action **Envoyer un message**, configurée depuis le [Panel](/dashboard/first/custom-commands){target=_blank} :
  ...
  ::tab{ label="Rôles" }
    ...
    *Lors de l'exécution de la commande, Draftbot attribuera un rôle, puis le retirera après un temps de votre choix.*
    ::collapse{ label="Afficher un exemple :" }
      Depuis le [Panel](/dashboard/first/custom-commands){target=_blank}, il est très facile de régler avec précision la durée souhaitée :
    ...
    *Lors de l'exécution de la commande, Draftbot retirera le/les rôles sélectionnés.*
    ::collapse{ label="Afficher un exemple :" }
      ...
      ![Exemple d'action Retirer des rôles](../assets/commandes-personnalisees/ex_cmd_perso_retirer_role.png)
...
::tabs
  ...
  ::tab{ label="Depuis le Panel" }
    ::hint{ type="info" }
      Depuis le [Panel](/dashboard/first/custom-commands){target=_blank}, vous pouvez faire apparaître ces options en cliquant sur "Avancé", en dessous de vos actions.
      ![Développer le menu des options avancées](../assets/commandes-personnalisees/cmd_perso_avance.png)
    ::
  ::
::
...
::tabs
  ::tab{ label="Restrictions" }
    ...
    ::hint{ type="info" }
      ...
      ![Dans cette configuration, si j'ajoute des rôles/salons, ils n'auront pas accès à la commande.](../assets/commandes-personnalisees/cmd_perso_avance_switch_restr.png)
    ::
  ::
  ...
  ::tab{ label="Mode lent" }
    ...
    - `Nombre d'utilisations` : le nombre d'utilisations par le même utilisateur avant de déclancher un temps de rechargement
    - `Durée du mode lent` : la durée de rechargement avant que l'utilisateur puisse utiliser de nouveau la commande.

    ![Le Panel offre une interface intuituve pour régler le mode lent. ](../assets/commandes-personnalisees/cmd_perso_avance_cooldown.png)

    ::hint{ type="success" }
      Si vous êtes modérateur/administrateur, vous pouvez réinitialiser votre propre nombre d'utilisations d'une commande ! Pour cela, rendez-vous sur le [Panel](/dashboard/first/custom-commands){target=_blank}, cliquez sur "**Modifier**" à côté de la commande en question.\
      Un bouton "**Réinitialiser mon mode lent**" apparaîtra en bas à gauche de l'écran :

      ![Réinitialiser son mode lent depuis le panel](../assets/commandes-personnalisees/cmd_perso_avance_reset_cd.png)
    ::
  ::
  ...
  ::tab{ label="Conditions" }
    ...
    ::
    ::hint{ type="success" }
      Grâce au [Panel](/dashboard/first/custom-commands){target=_blank}, vous pouvez configurer tout ça en quelques clics :
...
::collapse{ label="Variables disponibles" }
  ...
  **Niveaux** :
  > `{level}` ➜ Niveau du membre (si le système de [niveaux](/docs/modules/niveaux){target=_blank} est activé)
  >
  > `{level.rank}` ➜ Place du membre (dans le classement des niveaux)
  >
  > `{money}` ➜ Argent du membre (si le système d'[économie](/docs/modules/economie){target=_blank} est activé)
  >
  > `{money.rank}` ➜ Place du membre (dans le classement de l'économie)
  >
  > `{birthday}` ➜ Date d'anniversaire du membre (si ce [système](/docs/modules/anniversaire){target=_blank} est activé)
