...
::hint{ type="info" }
  Le contenu des suggestions (titre, description et commentaires) peut être automatiquement modéré par le module de [modération du contenu utilisateur](/docs/modules/auto-moderation#modération-du-contenu-utilisateur). Si ce module est activé, les suggestions contenant du vocabulaire interdit, des invitations Discord ou des liens non autorisés seront bloquées ou censurées selon le mode configuré.
::
