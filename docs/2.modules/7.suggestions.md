...
1. **Statut de suggestion** : Le statut que la suggestion doit obtenir : acceptées, refusées, en attente
...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    ![Bouton "Tri et recherche", via la commande/config](../assets/suggestions/config_recompenses.png)
    ...
    | Variable | Description | Exemple |
    |----------|-------------|---------|
    | `{reward}` | Récompense venant d'être reçue | Objet Canard x3 |
    | `{target.count}` | Nombre de suggestions | 5 |
    | `{target.type}` | Type de statut de suggestion | suggestion acceptée |

    *En plus des [autres variables](/docs/autres/variables) déjà disponibles globalement !*

    ![Menu "Annonce de récompenses", via la commande/config](../assets/suggestions/commande_config_annonce_recompenses.png)
  ::
  ...
  ::tab{ label="Via le Panel" }
    ...
    ![Bouton "Création d'une récompense", depuis le panel](../assets/suggestions/panel_creation_recompense.png)
  ::
::
