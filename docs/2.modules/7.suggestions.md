...
::tabs
  ...
  ::tab{ label="Via le Panel" }
    ...
    Depuis le Panel DraftBot, vous pouvez activer ou désactiver le module d'uņ simple clic sur le bouton affiché à côté du nom du module.
  ::
::
...
::tabs
  ::tab{ label="Via la commande /config" }
    Ce bouton permet d'ouvrir le menu de gestions des couleurs.
...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    DraftBot peut créer automatiquement un fil de discussion attaché chaque suggestion, permettant à l'auteur de la suggestion, et aux modérateurs, d'interagir ensemble.
