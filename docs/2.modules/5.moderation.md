...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    ::hint{ type="info" }
    Vous avez également la possibilité de déterminer un rôle qui pourra appliquer cette sanction sans nécessaire avoir la permission pour. Exemple : Le rôle A peut appliquer l'expulsion pour publicité sans qu'il  n'ait la permission d'expulser des membres
    ::
  ...
  ::tab{ label="Via le panel" }
    ...
    2. Vous pouvez également choisir quels rôles peut accéder à cette sanctions sans avoir la permission requise Exemple : Le rôle A peut appliquer l'expulsion pour publicité sans qu'il  n'ait la permission d'expulser des membres

    3. Vous n'avez ensuite qu'à sauvegarder votre sanction et le tour est joué !

    ![Création d'une sanction prédéfinies](../assets/moderation/creation_sanctions_predefinis.png)
...
Bravo ! vous avez configuré vos sanctions prédéfinies ! Vous pouvez ajouter le système de **signalement** pour les utiliser de manière plus efficace !
::card
---
title: Signalement
