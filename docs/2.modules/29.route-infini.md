...
- **Alternance obligatoire** : Vous ne pouvez pas envoyer deux nombres consécutifs. Un autre membre doit compter entre vos deux messages (cette règle peut être configurée par les administrateurs)
...
![Interface de la Route de l'Infini](../assets/route-infini/command_infiniteroad.png)
...
- **Pas de pénalités** : Ne subissent jamais les conséquences des erreurs (retrait d'argent, rôle temporaire, reset)
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Activation de la Route de l'Infini depuis le Panel](../assets/route-infini/panel_enable.png)
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ::collapse{ label="Statut" }
      Ce bouton vous permet d'activer ou désactiver le système de la Route de l'Infini.

      ![Bouton "Statut" via la commande /config](../assets/route-infini/config_status.png)
    ::

    ::collapse{ label="Salon" }
      Permet de choisir ou modifier le salon dédié à la Route de l'Infini. Le salon peut être un salon textuel, d'annonces ou un fil de discussion.

      ::hint{ type="warning" }
        DraftBot doit avoir les permissions **Voir le salon**, **Envoyer des messages** et **Ajouter des réactions** dans le salon choisi.
      ::
    ::
    ...
    ::collapse{ label="Animateurs" }
      ...
      - Envoyer des messages invalides sans pénalité
      - Modifier le nombre en cours
      - Gérer les récompenses

      ![Menu "Animateurs" via la commande /config](../assets/route-infini/config_animators.png)
    ::

    ::collapse{ label="Design" }
      Ce menu vous permet de personnaliser l'apparence des messages :
      - **Type de message** : Classique, DraftBot ou Webhook
      - **Format d'affichage** : Message ou Embed (modes DraftBot/Webhook)
      - **Réaction de validation** : Emoji ajouté automatiquement (mode Classique)
      - **Format personnalisé** : Texte avec variables (<:icon_premium:1096140508625125417>)
      - **Couleur de l'embed** : Personnalisation de la couleur (<:icon_premium:1096140508625125417>)

      ![Menu "Design" via la commande /config](../assets/route-infini/config_design.png)
    ::

    ::collapse{ label="Discussions" }
      Gérez les options relatives aux discussions :
      - Autoriser/interdire les discussions après le nombre
      - Autoriser/interdire les sauts de ligne dans les discussions
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Création de récompense depuis le panel](../assets/route-infini/panel_reward_create.png)
  ::
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ![Création de récompense via /config](../assets/route-infini/config_reward_create.png)
  ::
::
