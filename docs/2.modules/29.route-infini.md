...
- **Autoriser plusieurs messages à la suite** : Permet d'envoyer 2 à 20 nombres consécutifs sans qu'un autre membre ait à compter entre vos messages. Sinon, un autre membre devra compter entre vos deux messages
...
Vous pouvez configurer les récompenses multiples afin qu'un membre ne soit pas toujours avantagé.

| Type | Description |
|------|-------------|
| Pas deux fois de suites | Si le même membre déclenche le palier deux fois d'affilée, la récompense ne lui est pas redonnée.
| Limite de gain | Un membre ne peut remporter cette récompense qu'un nombre de fois défini. |
...
Vous souhaitez ajouter un effet de surprise à vos membres, alors les récompenses mystères vous permettent de cacher les récompenses que vous souhaitez attribuer à vos membres et de la remplacer par ":gift: Récompense mystère".

![Affichage d'une récompense mystère via la commande /route-infini](../assets/route-infini/bot_mystery_gift.png)

Vous pouvez configurer les récompenses mystère via :

::tabs
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/infinite-road)

    Vous pouvez ensuite commencer par créer une récompense. Quand vous sélectionnerez le type de récompense, une nouvelle option apparaît : « Masquer dans /route-infini ».

    ![Nouvelle partie afficher dans la configuration de récompense du panel](../assets/route-infini/panel_hide_infinity_road.png)

    Sélectionnez les composants que vous souhaitez cacher à vos membres.

    ::hint{ type="warning" }
      N'oubliez pas d'appuyer sur le bouton "créer"
    ::
  ::

  ::tab{ label="Via la commande /route-infini" }
    Si vous êtes un animateur de la route de l'infini ou un administrateur, vous pourrez appuyer sur le bouton "gérer les récompenses". De là, vous pourrez :
    ...
    Si vous modifiez une récompense, vous aurez un bouton "élément masqué", il vous suffira de cliquer sur celui-ci et de sélectionner les éléments que vous souhaitez masquer.

    ![Bouton "élément masqué"](../assets/route-infini/config_hide_element.png)
  ::
::
...
Vous pouvez personnaliser votre propre message de réinitialisation avec les variables suivantes :
- `{user}` : l’utilisateur qui a commis la faute
- `{infiniteroad.error_type}` : le type de faute commise par l’utilisateur
- `{infiniteroad.number}` : le prochain numéro
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Activez l’option "Réinitialiser la Route de l’Infini en cas d’erreur".

    ::hint{ type="success" }
      Une nouvelle partie apparait avec un message modifiable.
    ::
    ![Configuration du message de réinitialisation](../assets/route-infini/panel_reset__message.png)

    Modifie le message comme tu veux avec les variables.
  ::

  ::tab{ label="Via la commande /config" }
    Si vous êtes administrateur, vous pouvez utiliser la commande : `</config système:Route de l'Infini>`.

    Ensuite, appuyez sur le bouton "Erreur", puis sur "Message de réinitialisation ".

    ![Image de la réponse de DraftBot après avoir appuyé sur le bouton « Message de réinitialisation »](../assets/route-infini/bot_reset_message.png)
  ::
::

::hint{ type="info" }
  Vous pouvez également utiliser le markdown.
::
