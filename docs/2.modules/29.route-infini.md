...
Vous pouvez configurer les récompenses multiples pour faire en sorte qu'un membre ne soit pas toujour avantager :
| Type | Description |
|------|-------------|
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Quand vous sélectionnerez le type de récompense, une nouvelle partie aparait : "Masquer dans /route-infini"
    ![Nouvelle partie afficher dans la configuration de récompense du panel](../assets/route-infini/panel_hide_infinity_road.png)
    Sélectionner les composant que vous souhaitez cachez à vos membres
  ...
  ::tab{ label="Via la commande /route-infini" }
    ...
    - Réinisialiser **toutes** les récompenses
...
Tu peux personnalisé ton propre message de réïnitialisation avec les variables :
- `{user}` : L'utilisateur qui a fais la faute
- `{infiniteroad.error_type}` : Le type de faute que l'utilisateur à commis
...
::tabs
  ::tab{ label="Via le panel" }
    ...
    activer l'option : "Réinitialiser la route de l'infini en cas d'erreur".

    ::hint{ type="success" }
      une nouvelle partie aparait avec un message modifiable.
    ::
    ![Configuration du message de réïnitialisation](../assets/route-infini/panel_reset__message.png)

    Modifie le message comme tu veux avec les variables.
  ::

  ::tab{ label="Via la commande /config" }
    si tu es administrateur, tu peux faire la commande : \</config système:Route de l'Infini>

    ensuite, appuis sur le bouton "erreur" puis sur "message de réïnitialisation".
    ![image de réponse de draftbot après avoir appuyer sur le bouton "message de réïnitialisation"](../assets/route-infini/bot_reset_message.png)
  ::
::

::hint{ type="info" }
  tu peux également utiliser les markdowns.
::
