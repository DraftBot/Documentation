...
Chaque membre doit envoyer le nombre suivant par rapport au précédent.
Si une erreur est détectée (mauvais nombre, messages consécutifs, format invalide), **DraftBot** applique automatiquement les règles configurées par les administrateurs.

La gestion du comptage est entièrement assurée par **DraftBot**, qui analyse chaque message envoyé dans le salon configuré.

> ***DraftBot** surveille en continu le salon.
À chaque message, il vérifie le nombre, l'auteur, le format et les règles actives avant de valider ou refuser le comptage.*
