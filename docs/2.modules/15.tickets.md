---
title: Tickets
description: Offrez la possibilité à vos membres de créer des tickets leur permettant de recevoir une assistance de l'équipe de votre serveur, en toute confidentialité.
navigation.icon: 'twemoji:admission-tickets'
contributors: ['toutouille-db', 'imroxxor', 'clement669']
updatedAt: '2025-04-22'
---

### Créer un ticket

Si vous rencontrez des difficultés et souhaitez en discuter avec le staff compétent, dans un salon confidentiel, vous pouvez ouvrir un ticket. Pour ce faire, deux méthodes sont à votre disposition :
...
Une fois votre ticket envoyé, un salon est créé par DraftBot pour que vous puissiez échanger avec les modérateurs les plus à même de vous apporter l'aide dont vous avez besoin.

::hint{ type="info" }
  En dehors des administrateurs du serveur, l'équipe de modération et vous êtes les seuls habilités à accéder à ce salon.
::

Après que votre ticket a été créé, il peut y avoir un certain délai avant qu'un modérateur ne prenne contact avec vous. Soyez patient, et n'hésitez pas à commencer à expliquer la situation par avance !
...
Une fois le sujet résolu, vous pouvez clôturer le ticket. Lorsque votre choix est confirmé, le salon sera fermé par DraftBot.
...
En outre, si la fonction "Validation des tickets" est activée, c'est également dans ce salon que les modérateurs de tickets pourront valider (ou non) les demandes d'ouverture de ticket.
...
::tabs
  ...
  ::tab{ label="Activation via le panel" }
    ...
    Depuis le panel DraftBot, vous pouvez activer le système de tickets en vous rendant dans la section "Tickets", et en cliquant sur le bouton pour l'activer.
