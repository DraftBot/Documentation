...
Les boutiques permettent à vos membres d'acheter des articles mis en place par vos soins !
...
![Interface de la boutique](../assets/economie/shop.png)

::hint{ type="info" }
  Vous pouvez créer jusqu'à **1 boutique** avec jusqu'à **500 articles**. Les serveurs [premium](/premium) peuvent créer jusqu'à **5 boutiques** !
::

### Naviguer dans une boutique

Pour accéder aux boutiques du serveur, utilisez la commande \</boutique>.
...
Deux styles sont disponibles pour personnaliser l'expérience de vos boutiques :

| Style | Comportement |
|-------|--------------|
| **Classique** | Le message de la boutique reste affiché indéfiniment. Idéal pour une navigation tranquille. |
| **Marché noir** | Le message de la boutique disparaît automatiquement après 60 secondes d'inactivité. Parfait pour créer une ambiance exclusive. |

### Gestion des stocks

Les serveurs [premium](/premium) peuvent définir un **stock limité** pour chaque article. Une fois le stock épuisé, l'article ne peut plus être acheté jusqu'à ce que vous réapprovisionniez le stock manuellement.
...
### Argent par message (activité textuelle)
...
- **Les fils de discussion (threads)** : créés dans les salons textuels ou annonces. Leur gain d'argent peut être activé/désactivé via l'option **"Gain d'argent dans les fils de discussions"**.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ::collapse{ label="Argent" }
      ...
      - Activer / Désactiver le gain d'argent dans les fil
      ...
      ![Menu "Argent" via la commande /config](../assets/economie/config_bot_money.png)
    ::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Menu de création de boutique](../assets/economie/panel_bot_shops.png)
...
| **Style** | Choisissez entre "Classique" ou "Marché noir" |  |
| **Bannière** | Ajoutez une image de bannière en haut de la boutique (800x137px recommandé) | <:icon_premium:1096140508625125417> |
| **Couleur** | Personnalisez la couleur de l'interface de la boutique | <:icon_premium:1096140508625125417> |
| **Tri par défaut** | Définissez l'ordre de tri par défaut des articles | <:icon_premium:1096140508625125417> |
| **Conditions d'accès** | Restreignez l'accès à la boutique selon des conditions (rôles, niveaux, etc.) | <:icon_premium:1096140508625125417> |
...
- **Stock** : Quantité disponible (illimité par défaut, limité pour les serveurs premium)

### Types d'articles détaillés
...
::hint{ type="info" }
  ...
  ![Utilisation de la commande /dropargent](../assets/economie/bot_dropbot_money.png)
::
...
Vous débutez avec le système d'économie ? Suivez ces étapes simples :

1. **Activez le système** depuis le [panel](/dashboard/first/economy) ou via \</config>
2. **Configurez les gains d'argent** : Ajustez les valeurs min/max pour les messages (par défaut 15-25 💰)
3. **Créez une boutique** : Donnez-lui un nom et choisissez son style
4. **Ajoutez des articles** : Créez des récompenses motivantes pour vos membres
5. **Optionnel** : Affinez avec des listes blanches/noires, multiplicateurs et options avancées

::hint{ type="success" }
  **Astuce** : Commencez simple ! Activez le système avec les paramètres par défaut, créez une boutique avec quelques articles, et ajustez ensuite selon les besoins de votre communauté.
::

## Migration depuis UnbelievaBoat

Si vous utilisiez déjà un système d'économie via UnbelievaBoat, vous avez la possibilité d'importer l'argent de vos membres au système d'économie de DraftBot !
