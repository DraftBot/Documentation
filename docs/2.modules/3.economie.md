...
::hint{ type="success" }
  ...
  Le système d'économie mis en place par DraftBot est incroyablement complet, et peut être lié aux autres fonctionalités DraftBot (comme les **[commandes personnalisées](/docs/modules/commandes-personnalisees){target=_blank}, l'[inventaire](/docs/modules/inventaire){target=_blank}, les [anniversaires](/docs/modules/anniversaires){target=_blank}, les [niveaux](/docs/modules/niveaux){target=_blank}** ...).
::
