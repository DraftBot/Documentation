...
---
## Configuration
Le système d'économie mis en place par DraftBot est incroyablement complet, et peut être lié aux autres fonctionalités DraftBot (**[commandes personnalisées](/docs/modules/commandes-personnalisees), [inventaire](/docs/modules/inventaire), [anniversaires](/docs/modules/anniversaires), [niveaux](/docs/modules/niveaux)** ...).\
Pour en tirer le meilleur parti, il faut commencer par le commencement : la configuration.

### 1. Configurer le système d'économie

::hint{ type="success" }
  Vous pouvez configurer le système d'économie depuis le [__panel__](/dashboard/user) ou directement sur Discord, via la commande \</config>.
::
