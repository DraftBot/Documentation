...
::hint{ type="success" }
  ...
  Le système d'économie mis en place par DraftBot est incroyablement complet, et peut être lié aux autres fonctionalités DraftBot (**[commandes personnalisées](/docs/modules/commandes-personnalisees){target=_blank}, [inventaire](/docs/modules/inventaire){target=_blank}, [anniversaires](/docs/modules/anniversaires){target=_blank}, [niveaux](/docs/modules/niveaux){target=_blank}** ...).
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/economie){target=_blank}
    ...
    ::hint{type="warning"}
      Une fois vos modifications terminées, n'oubliez pas d'enregistrer vos modifications avec le bouton "Enregistrer" en bas de la page !
    ::

  ::
  ::tab{ label="Via la commande /config" }

    Voici les différents boutons à votre disposition :
...
::collapse{ label="Afficher / masquer la liste de fonctionnalités" }
  ...
  |**Argent de départ**                                             |Modiifer la somme de base reçue en rejoignant le serveur |
...
::collapse{ label="Afficher / masquer la liste des options" }
  ...
  |     **NOM**    | **EXPLICATION** |
  |------------|-------------|
  |**Argent par message**| EXPLICATION |
...
::hint{ type="info" }
  Les fonctionnalités ou options accompagnées de l'icône <:icon_premium:1096140508625125417> sont exclusives, et réservées aux abonnés [✨DraftBot premium](https://www.draftbot.fr/premium){target=_blank_}. 
::
...
::hint{ type="info" }
  ...
  Le type de boutique est configurable depuis la commande \</config> ou en vous rendant dans la partie Boutique de [la page "Économie"](/dashboard/first/economie){target=_blank} du panel.
...
Pour plus d'informations, rendez-vous sur la page des [inventaires](/docs/modules/inventaire){target=_blank}.
