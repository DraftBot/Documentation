...
::hint{ type="info" }
  Les méthodes comme l'**application d'authentification** et la **clé d'accès** sont considérés comme sécurisés. Les méthodes comme le **code PIN** et le **mot de passe** sont des alternatives moins sécurisées et qui peuvent être restreintes par les administrateurs du serveur.
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ::hint{ type="info" }
      Vous pouvez sécuriser jusqu'à 2 rôles. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent en sécuriser jusqu'à 10.
    ::
  ::
...
Ceux qui n'auront pas **d'authentification configurée** auront **24 heures** pour en configurer une, sans quoi leur accès aux permissions sera désactivé.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ::hint{ type="info" }
      Vous pouvez sécuriser jusqu'à 10 membres. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent en sécuriser jusqu'à 25.
    ::
  ::
...
1. **Désactivation planifiée :** Date et heure à laquelle le membre perdra ses rôles d'accès <:icon_premium:1096140508625125417>
2. **Période d'inactivité :** Le membre perdra ses rôles d'accès s'il ne se connecte pas pendant une durée définie, configurable en jours, semaines ou mois. <:icon_premium:1096140508625125417>

::hint{ type="info" }
  Les fonctionnalités accompagnées de l'icône <:icon_premium:1096140508625125417> sont réservées aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>.
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ::hint{ type="info" }
      Vous pouvez créer jusqu'à 2 accès automatiques. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent aller jusqu'à 10.
    ::
  ::
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ::hint{ type="info" }
      Vous pouvez créer jusqu'à 2 accès automatiques. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent aller jusqu'à 10 accès automatiques.
    ::
  ::
::
...
Permet de consulter l'historique des activités durant les sessions, comme les [giveaways](/docs/modules/giveaways), les [actions de modération](/docs/modules/moderation), et bien d'autres...
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    - **Période** : Sessions actives, dernières 24 heures, derniers 7 jours, derniers 30 jours
    ...
    - **Niveau de danger** : Élevé, moyen, faible
    - **Membre** : Un membre spécifique
    - **Sessions sans actions** : Masquer ou afficher les sessions où aucune action n'a été effectuée

    Un bouton **Filtres avancés** permet d'accéder à des filtres supplémentaires :
    - **Méthode d'authentification** : Application d'authentification (TOTP), clé d'accès (WebAuthn), code PIN ou mot de passe
    - **Rôle sécurisé** : Sessions liées à un rôle sécurisé en particulier
    - **Cible de l'action** : Sessions ayant agi sur un membre ou un objet précis
    - **Action spécifique** : Filtrer sur un type d'action donné

    Les sessions peuvent également être **triées** par :
    - Plus récentes
    - Plus d'actions
    - Plus dangereuses
    - Plus longues
    ...
    - Les **changements de pseudonyme** effectués pendant la session
  ::
...
- **Actions effectuées :** Les actions que l'utilisateur a effectuées grâce à ces permissions, ainsi que les éventuels **changements de pseudonyme** survenus pendant la session
...
**Période :**
1. **Toutes les périodes :** Affiche toutes les sessions.
2. **Sessions actives :** Affiche les sessions actuellement en cours.
3. **Dernières 24 heures**, **7 jours** ou **30 jours :** Restreint l'historique à une fenêtre temporelle.

**Filtrer par catégorie d'action :** Permet de filtrer les sessions par type d'action effectuée (Modération, Rôles, Salons, etc.).

**Filtrer par niveau de danger :** Élevé, moyen ou faible, selon la sensibilité des actions effectuées dans la session.

**Sélectionner un membre :** Permet de sélectionner l'un des membres qui apparaît dans les historiques.

**Masquer les sessions sans actions :** Permet d'isoler uniquement les sessions où au moins une action a été effectuée.

::hint{ type="info" }
  Un bouton **Filtres avancés** ouvre des options supplémentaires : filtrer par **méthode d'authentification** (A2F, clé d'accès, PIN, mot de passe), par **rôle sécurisé** spécifique, par **cible** d'une action ou par **action précise**.
::

**Trier les sessions :** Vous pouvez choisir l'ordre d'affichage : plus récentes, plus d'actions, plus dangereuses, ou plus longues.
