...
Avec les rôles sécurisés, les permissions sensibles ne sont **attribuées que temporairement** après une authentification via une application d'authentification ou une clé d'accès (empreinte digitale, reconnaissance faciale, code Windows ou clé physique).
...
Les clés d'accès (WebAuthn) permettent d'utiliser l'empreinte digitale, la reconnaissance faciale, le code Windows Hello ou une clé physique pour s'authentifier.
...
::tabs
  ::tab{ label="Via le panel" }
    ...
    ![Aperçu d'un rôle à risque](../assets/roles-securises/panel_risky_role.png)
...
Vous pourrez également configurer les **réglages** suivants :
...
::collapse{ label="Options avancées (Premium)" }
...
Les utilisateurs à protéger **sans authentification configurée** auront **24 heures** pour s'authentifier via un MP envoyé par **DraftBot**, sans quoi ils perdront l'accès à leurs permissions.
...
- **Clé d'accès :** Permet d'utiliser l'empreinte digitale, la reconnaissance faciale, le code Windows ou une clé physique. Un lien s'ouvrira dans votre navigateur pour enregistrer votre clé.
...
::
- **Ajouter une clé d'accès :** Permet d'ajouter une clé d'accès (empreinte digitale, reconnaissance faciale, code Windows ou clé physique). Vous pouvez en enregistrer plusieurs.
...
Certaines fonctionnalités des rôles sécurisés sont réservées aux serveurs [Premium](/docs/modules/premium) :

- **Désactivation planifiée :** Retire automatiquement l'accès après une durée définie. Utile pour le staff temporaire.
- **Période d'inactivité :** Désactive automatiquement l'accès des membres qui ne se connectent plus.
- **Limites augmentées :** Nombre de rôles sécurisés, de membres par rôle et d'accès automatiques plus élevés.
- **Durées de sessions personnalisées :** Plus de flexibilité dans la configuration des durées de sessions.
