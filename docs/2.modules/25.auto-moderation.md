...
::collapse{ label="Liens externes" }
  Ce module permtet d'interdire la publication de liens sur votre serveur.

  L'option **nom de domaines autorisés/interdits** permet de n'interdire que certains liens ou d'en ignorer (sélectionnez le mode adapté) en précisant le nom de domaine ; par exemple `draftbot.fr`.
...
::collapse{ label="Émojis excessifs" }
  ...
  Le **nmbre d'émojis maximum** est le nombre maximal d'émojis qu'un message peut comporter.
::

::collapse{ label="Mentions excessives" }
  Ce module permet d'empêcher le spam de mentions.

  L'**intervalle de temps** est la durée pendant laquelle le nombre de mentions envoyés par un membre ne doit pas dépasser la limite définie ensuite.
...
::collapse{ label="Spam de messages" }
  ...
  L'**intervalle de temps** est la durée pendant laquelle le nombre de messages envoyés par un membre ne doit pas dépasser la limite définie ensuite.
...
- **Mode silencieux** : Si activé, DraftBot ne répondra pas à l'infraction avec un message expliquant l'infraction.
