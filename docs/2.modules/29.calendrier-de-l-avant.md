...
::tabs
  ::tab{ label="Rôle" }
    ...
    ::hint{ type="info" }
      **Rôle Cumulatif** : Le membre conserve tous les rôles obtenus via le calendrier.

      **Rôle Unique** : Seul le dernier rôle unique obtenu est conservé. Si le membre possède déjà un rôle unique d'une case précédente, ce rôle sera **automatiquement retiré** lors de l'ouverture d'une nouvelle case avec un rôle unique.
    ::
