...
::tabs
  ::tab{ label="Via la commande /config" }
    ...
    ***Ciblé** : Envoi du message à un jour et une heure ciblée.*
    ...
    ::hint{ type="warning" }
      Ce message est assez peu personnalisable. C'est pourquoi nous vous conseillons de créer un message entièrement customisé depuis le [panel](/dashboard/first/messages){target=_blank} et de le sélectionner par la suite grâce à son [identifiant](https://www.draftbot.fr/docs/autres/recuperer-un-identifiant#identifiant-dun-message){target=_blank}.
    ::
