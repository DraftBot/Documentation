...
- **Champs :** Vous pouvez insérer ici plusieurs zones de texte organisées (titre et contenu). Les blocs "champs" sont séparés les uns des autres par un interligne plus grand.
    ...
    ::hint{ type="info" }
      Le nombre de champs est limité à `25` par embed. Il ne peut pas y avoir plus de `1024` caractères dans un champs.
...
- <:spec:1359820882788356176> : Ce bouton vous permet de prévisualiser le rendu que votre message aura sur Discord une fois envoyé/modifié.
- <:palette:1359820880934469806> : Ce bouton vous permet de changer la couleur de l'embed.
...
Pour ajouter un profil, il faut cliquer sur la photo de profil de DraftBot pour faire apparaître un crayon pour en créer un profil ou cliquer sur le bouton <:setting:1389695793589387374> en haut à droite de votre message. Vous pourrez alors choisir un profil déjà créé ou en créer un nouveau.
