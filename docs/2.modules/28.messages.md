...
::collapse{ label="Ajouter ou retirer des rôles" }
  ...
  - Unique : L'utilisateur ne peut avoir qu'un seul rôle du message. S'il choisit un autre bouton qui donne un autre rôle, son rôle précédent lui sera retiré.
  Vous devez ensuite définir quels sont les rôle à donner ou retirer. Vous pouvez également choisir si les rôles donnés sont temporaire ou non. Si le rôle est temporaire, vous devez définir la durée pendant laquelle l'utilisateur conservera son rôle.
  ...
  ::
  Vous pouvez faire acheter via un bouton un article de votre [boutique](/docs/modules/economie#la-boutique). Vous devez ajouter l'article à faire acheter, l'utilisateur l'achètera au **même prix** que celui proposé à la vente.::collapse{ label="Proposer une suggestion"}
...
::collapse{ label="Redirger vers un lien" }
  ...
  ::collapse{ label="Rediriger vers un lien" }choisir la couleur du bouton. Celui-ci sera obligatoirement gris.
...
Vous pouvez ainsi nommer votre bouton (visible par tous les utilisateurs), y ajouter un emoji (de votre serveur ou ceux de discord) et choisir sa couleur.
Vous devrez ensuite sélectionner l'action voulue. Si vous souhaitez créer plusieurs boutons, recommencez l'opération.
...
Le mode simple vous permet de faire quelques actions plus facilement et rapidement.
...
Une fois choisi, le menu se développera pour vous permettre de configurer l'ensemble des sélecteurs avec cette action. Vous devrez alors à minima nommer votre option, lui ajouter un émoji et régler l'action précise. Vous pouvez également ajouter une description ou personnaliser le nom qui apparaît pour l'utilisateur pour les actions ajout & retrait des rôles, d'ouverture de ticket et d'achat d'articles de boutique.r une description en appuyant sur la flèche orange.
...
Le mode avancé vous permet de personnaliser chaque option du sélecteur. Vous pouvez ainsi créer un seul sélecteur pour différentes actions.## Créer des profils
...
## Envoyer un message
Pour ajouter un profil, il faut cliquer sur la photo de profil de DraftBot pour faire apparaître un crayon pour en créer un profil. Vous pourrez alors choisir un profil déjà créé ou en créer un nouveau.Pour envoyer votre message après l'avoir rédigé sur le panel vous devrez choisir un salon d'envoi en haut de la page puis cliquer sur bouton `Envoyer`
...
Vous pouvez récuperer un message en appuyant sur le bouton orange `Récupérer un message` situé en haut à droite de la page. Un menu apparaîtra alors pour retrouver le message via l'identifiant du message et son salon d'origine. Vous pouvez également retrouver le message via le lien du message.
