...
::hint{ type="info" }
  Vous pouvez mettre jusqu'à 2000 caractères dans votre contenu.
::
...
- **Champs :** Vous pouvez insérer ici plusieurs zones de texte organisées (titre et contenu). Les blocs "champs" sont séparés les uns des autres par un interligne plus grand.
    ...
    ::hint{ type="info" }
      Le nombre de champs est limité à `25` par embed. Vous pouvez mettre jusqu'à `1024` caractères dans la valeur du champ.
...
- Un **émoji** de votre choix, qui s'affichera sur le bouton, à gauche du nom. (Les émojis de votre serveur sont acceptés, mais les émojis animés ne le sont pas) ;
