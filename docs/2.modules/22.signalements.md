...
Pour effectuer un signalement, il suffit d'exécuter la commande \</signaler>. Vous sélectionnez l'utilisateur ou le message qui ne respecte pas les règles que vous avez instaurées.
...
::tabs
  ::tab{ label="Depuis la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Signalement` ➔ `Rôle mentionné`.
  ::
