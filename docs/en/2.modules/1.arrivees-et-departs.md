...
::tabs
  ::tab{ label="Via the /config command" }
    ...
    ::collapse{ label="Variable" }
      A variable is a piece of text that changes depending on the person, server, channel, or time. Here are those available in DraftBot's welcome messages.
    ...
    ::hint{ type="info" }
      You can choose between three colors and the default one that will make the image transparent (with the "None" button).
