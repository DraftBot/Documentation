---
slug: /engagement/salons-vocaux-temporaires
...
contributors: ['rababio4579', 'theorik', 'elbaguetto', 'thecreatorn3ant', 'imroxxor', 'lyvendev']
updatedAt: '2026-08-05'
---

## Utiliser des salons temporaires

Pour obtenir votre salon vocal, il vous faudra rejoindre le **hub** : un salon vocal un peu particulier, qui ne sert pas à discuter mais à créer votre propre salon. Dès que vous le rejoignez, DraftBot vous déplace automatiquement dans un nouveau salon rien qu'à vous. Il se nomme par défaut `➕ Créer votre salon`.

Lorsque vous créerez un salon vocal temporaire, vous aurez la possibilité de configurer votre salon par le biais d'un [message](#utilisation-de-lembed-de-réglages) *(option désactivable)*.
...
![Démonstration du système](./assets/temporary-voice-channels/view_guide.gif)
...
![Illustration des boutons du configurateur](./assets/temporary-voice-channels/view_button.png)
...
- **Liste Blanche** : Cette fonction permet d'ajouter des membres qui seront **autorisés à rejoindre** le salon quel que soit le mode (Ouvert, Fermé, Privé) et recevront toutes les permissions micro, vidéo et soundboards.
...
- **Statut** : Cette fonction permet d'autoriser/refuser la définition du statut du salon vocal par les participants.

::hint{ type="info" }
  Après chaque changement, **DraftBot** vous confirme la nouvelle permission dans un message visible par vous seul. Ce message liste également les membres qui n'y sont pas soumis, parce qu'un rôle, la permission **Administrateur** ou une dérogation du salon leur accorde (ou leur refuse) déjà cette permission par ailleurs. Pour le **Micro**, il rappelle aussi que les membres déjà connectés devront se reconnecter, Discord ne permettant pas de faire autrement.
::

- **Réglages** : Cette fonction ouvre un formulaire permettant de modifier le **nom** du salon vocal, sa **limite de membres** (de 0 à 99, où 0 signifie aucune limite) et sa **région vocale**. Les options présentes dans le formulaire dépendent de la configuration choisie par l'administrateur (voir [Options de réglages](#options-de-réglages)). Cela permet aux membres de personnaliser leur salon **sans avoir besoin de la permission "Modifier le salon"** dans les permissions du créateur, ni de l'**A2F** requise par Discord sur les serveurs communautaires.
...
- **Sauvegardes** <:icon_premium:1096140508625125417> : Cette fonction permet de **sauvegarder** et **charger** des configurations complètes de salon vocal. Consultez la section [Sauvegardes de configuration](#sauvegardes-de-configuration) pour plus de détails.
...
::tabs
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/privaterooms)

    Vous devez vous rendre sur le panel puis dans la catégorie [Salons vocaux temporaires](/dashboard/first/privaterooms).

    ![Aperçu du panel](./assets/temporary-voice-channels/panel_hub.png)
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Créer un hub`.

    ![Aperçu de la commande /config](./assets/temporary-voice-channels/config_hub.png)
  ::
::

::hint{ type="info" }
  Par défaut, vous ne pouvez créer qu'un seul hub. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent en créer jusqu'à 10.
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Dans la liste des hubs, un bouton d'activation / désactivation est disponible à gauche du bouton `Modifier`.
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub`.

    Après avoir cliqué sur le bouton, vous verrez que votre hub est activé via le bouton vert intitulé `Système activé`.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Allez dans le panel, puis dans la catégorie [Salons vocaux temporaires](/dashboard/first/privaterooms).
    ...
    - **Sélectionner** : vous désignez une catégorie existante, par son nom ou son [identifiant](/docs/autres/recuperer-un-identifiant).
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Catégorie`.
...
### Changer le hub
Vous pouvez changer le salon vocal qui sert de hub, c'est-à-dire celui qui déclenche la création d'un salon temporaire lorsqu'un membre le rejoint.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Vous aurez le choix entre plusieurs options dans la rubrique "salon de création".

    - **Créer** : DraftBot met en place un nouveau salon vocal de création pour vous.
    - **Sélectionner** : vous désignez un salon vocal existant, par son nom ou son [identifiant](/docs/autres/recuperer-un-identifiant).
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Hub (salon)`.

    Après avoir cliqué sur le bouton, un message de confirmation apparaîtra. Cliquez sur `Oui` si vous souhaitez appliquer la modification.

    Vous aurez le choix entre :

    - **Nouveau salon** : DraftBot met en place un nouveau salon vocal de création pour vous.
    - **Utiliser un salon existant** : vous entrez le nom ou l'identifiant du salon vocal à utiliser.
  ::
::

### Format des salons vocaux

Pour rendre les salons vocaux plus esthétiques, vous pouvez personnaliser le format du salon vocal temporaire en y ajoutant des mots, des émojis et des variables.

::tabs
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/privaterooms)

    Allez dans le panel, puis dans la catégorie [Salons vocaux temporaires](/dashboard/first/privaterooms). Ensuite, cliquez sur le bouton `Modifier`.

    Vous avez la possibilité de mettre des variables parmi la liste ci-dessous :
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Nom des salons`.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Permissions par défaut`.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Permissions du créateur`.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Limite de membres`.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Salons permanents`.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Embed de réglages activé`.
...
### Rôles d'accès
Cette option permet d’autoriser un ou plusieurs rôles à rejoindre les salons vocaux temporaires par défaut. Ce sont les permissions de ces rôles qui sont modifiées lorsque le propriétaire change la visibilité de son salon (`Ouvert`, `Fermé`, `Privé`).

::hint{ type="info" }
  Vous pouvez sélectionner jusqu'à **5 rôles d'accès**, qui ne pourront pas être ajoutés aux listes blanche et noire par le propriétaire du salon.
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Vous aurez la possibilité de configurer depuis la catégorie "Rôles d'accès aux salons temporaires", en sélectionnant **@everyone** ou jusqu'à **5 rôles**.
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Rôles d'accès`.
    ...
    - **Rôles personnalisés** : Choisissez jusqu'à **5 rôles** que vous avez déjà mis en place. Si **@everyone** n'a pas accès à la catégorie ou au salon de création, **DraftBot** vous propose directement les rôles qui y ont accès.
  ::
::
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Rôles d'accès aux réglages`.
...
### Rôles modérateurs
Les membres possédant un rôle modérateur auront **automatiquement accès à tous les salons vocaux temporaires** et ne pourront **pas être ajoutés à la liste noire** par le créateur du salon. Ils peuvent également **renommer** n'importe quel salon vocal temporaire, indépendamment des [options de réglages](#options-de-réglages) configurées par l'administrateur.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Vous aurez la possibilité de configurer depuis la catégorie "Rôles modérateurs".
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Rôles modérateurs`.

    Vous pourrez ajouter ou supprimer des rôles modérateurs.
  ::
::


### Rôles intouchables
Les membres possédant un rôle intouchable ne pourront **pas être ajoutés à la liste blanche ni à la liste noire** par le créateur du salon. Contrairement aux rôles modérateurs, ils n'ont pas automatiquement accès aux salons.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Vous aurez la possibilité de configurer depuis la catégorie "Rôles intouchables".
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Rôles intouchables`.

    Vous pourrez ajouter ou supprimer des rôles intouchables.
  ::
::


### Permissions des membres en liste blanche
Cette option vous permet de décider si les membres de la liste blanche ont la possibilité d'accéder aux quatre permissions suivantes.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Permissions des membres en liste blanche`.
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Personnalisation des boutons de réglages`.
...
Vous pouvez choisir précisément les options que vos membres peuvent modifier depuis le bouton **Réglages** de l'embed. Les trois options disponibles sont :
...
::hint{ type="info" }
  Décocher une option la retire du formulaire de réglages : les membres ne pourront plus la modifier eux-mêmes.
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Vous trouverez l'option **Options de réglages** permettant de sélectionner les champs à afficher dans la modale de réglages.
  ::

  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Options de réglages`.

    Vous pourrez cocher ou décocher chacune des options à afficher dans la modale de réglages.
  ::
::
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Réquisition de salons autorisée`.
...
![Menu d'une sauvegarde](./assets/temporary-voice-channels/saves.png)
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    Exécutez la commande \</config>, puis sélectionnez depuis le menu déroulant `Salons vocaux temporaires` ➔ `Modifier un hub` ➔ `Sauvegardes`.
...
::hint{ type="info" }
  Cette fonctionnalité est réservée aux serveurs [premium](/premium) <:icon_premium:1096140508625125417> et doit être activée par un administrateur dans la configuration du hub.
::

Les sauvegardes permettent aux membres de **sauvegarder** et de **charger** des configurations complètes de leur salon vocal temporaire. Cela inclut :
...
::hint{ type="info" }
  Lorsqu'une sauvegarde par défaut est définie, chaque modification du salon sera conservée d'un salon à l'autre (changement de visibilité, de permissions, etc.).
::
