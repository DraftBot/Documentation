...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Vous devez vous rendre sur le panel puis dans la catégorie [Communautaire](/dashboard/first/community).
...
::collapse{ label="Liste des variables" }
  ...
  - `{random_word}` pour attribuer un mot aléatoire parmi une liste de mots de **DraftBot**.
  - `{custom_word}` pour attribuer un mot aléatoire parmi une liste personnalisable.

  ::hint{ type="info" }
    La variable `{custom_word}` doit être configurée depuis le panel.
  ::
::
