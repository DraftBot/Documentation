...
![Carte renvoyée par la commande /niveau](./assets/levels/bot_level_card.png)
...
| **Argent** | De l'argent du [système d'économie](/docs/engagement/economie) | 5000 pièces au niveau 15 |
| **Objet d'inventaire** | Un ou plusieurs [objets d'inventaire](/docs/engagement/inventaire) | 3x "Potion de boost" au niveau 25 |
| **Récompense personnalisée** | Une récompense que vous remettez manuellement | Code promo, goodies, etc. |

![Récompenses de niveau](./assets/levels/bot_rewards.png)

::hint{ type="info" }
  Vous pouvez créer jusqu'à **10 récompenses** gratuitement. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> n'ont **aucune limite** !
::
...
![Résultat de la commande /topniveau](./assets/levels/bot_toplevel.png)

::hint{ type="success" }
  Le classement est paginé : des boutons permettent de changer de page ou de sauter directement à une page donnée. Par défaut, 5 membres sont affichés par page ; l'option `top` permet d'en afficher de 1 à 25 (par exemple \</topniveau> `top:3`). Le classement s'arrête aux 100 premiers membres.
::
...
Si vous avez activé le **classement en ligne**, vous pouvez le consulter depuis le bouton **"Voir l'ensemble du classement"** de la commande \</topniveau>.

![Classement en ligne](./assets/levels/panel_leaderboard.png)
...
![Classement Discord automatique](./assets/levels/bot_leaderboard_auto.png)
...
| \</adminxp retirer membre> | Retirer de l'expérience à un membre. **Attention** : cela peut entraîner la perte de niveaux et des rôles de récompense associés. |
...
La méthode principale pour gagner de l'expérience est la **participation écrite** dans le serveur. Chaque message rapporte une quantité d'expérience **aléatoire**, comprise entre deux bornes configurables (par défaut entre **15 et 25 XP**).

::hint{ type="info" }
  Un **cooldown** (de 30 secondes par défaut, configurable) s'applique entre chaque gain d'XP pour un même membre. Cela évite le spam et encourage les conversations de qualité.
::
...
Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent activer le gain d'expérience en fonction de l'**activité vocale**. Chaque membre actif en vocal gagne une quantité d'expérience **aléatoire** (15 à 25 XP par défaut, configurable), à **intervalle régulier** (toutes les 2 minutes par défaut, configurable).
...
Afin de gérer votre serveur avec granularité, vous pouvez bloquer ou booster le gain d'expérience selon le rôle du membre ou le salon utilisé, comme détaillé ci-dessous.
...
- Si un membre cumule plusieurs rôles boostés, **seul le boost le plus élevé** sera appliqué.
...
::hint{ type="info" }
  **Exemple** : un membre possédant un rôle x2 qui écrit dans un salon x1.5 gagne x3 (2 × 1.5). À l'inverse, s'il possède deux rôles boostés (x2 et x1.5), seul le x2 sera appliqué.
::
...
| Option | Description | |
|--------|-------------|--|
| **Gain d'XP par message en vocal** | Trois modes disponibles pour les messages textuels envoyés dans les salons vocaux : **Tout le monde** (tout message rapporte de l'XP, peu importe si l'auteur est connecté au vocal), **Seulement si connecté au vocal** (l'XP n'est accordée que si le membre est également connecté au salon vocal), ou **Désactivé** (aucun message textuel dans les salons vocaux ne rapporte d'XP) | |
| **Longs messages comptent double** | Les messages de 250 caractères ou plus (seuil configurable entre 100 et 1500) rapportent le double d'XP | |
| **Voir les XP des autres utilisateurs** | Si désactivé, les membres ne peuvent consulter que leur propre niveau via \</niveau>, et la réponse devient éphémère (visible uniquement par celui qui l'utilise). Les administrateurs peuvent toujours consulter les niveaux de tous les membres | |
...
| **Personnalisation de la couleur** | Changez la couleur des embeds et cartes de niveau | <:icon_premium:1096140508625125417> |
...
::hint{ type="info" }
  ...
  ![Tableau des niveaux](./assets/levels/bot_experience_table.png)
::
...
| **Personnalisée** | Au choix | Définissez votre propre formule de progression | <:icon_premium:1096140508625125417> |
...
Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent définir un **niveau maximum** pour plafonner la progression. Une fois ce niveau atteint, les membres **ne gagnent plus aucune expérience** jusqu'à ce que le plafond soit relevé ou supprimé.
...
::hint{ type="info" }
  L'annonce de passage de niveau se déclenche **quelle que soit la source de l'XP** ayant permis de monter de niveau : messages, vocal, \</adminxp>, \</dropxp>, cadeau d'anniversaire, bingo, giveaway, Route de l'Infini, ou toute autre commande ou fonctionnalité octroyant de l'expérience.
::
...
| `{date}` | Date actuelle | 15/01/2025 |
| `{time}` | Heure actuelle | 14:30 |
| `{timestamp}` | Timestamp Discord dynamique | <t:1736947800:F> |
...
![Annonce d'un nouveau niveau](./assets/levels/bot_new_level_notif.png)
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Activation du système de niveaux depuis le Panel](./assets/levels/panel_enable.png)

    Ensuite, l'ensemble des fonctionnalités apparaît :

    ![Module Niveaux activé sur le Panel](./assets/levels/panel_home.png)
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ![Module Niveaux via la commande /config](./assets/levels/config_home.png)
    ...
    ::collapse{ label="Statut" }
      ...
      ![Menu "Statut" via la commande /config](./assets/levels/config_status.png)
    ...
    ::collapse{ label="Expérience" }
      ...
      ![Menu "Expérience" via la commande /config](./assets/levels/config_experience.png)
    ::

    ::collapse{ label="Récompenses de niveaux" }
      Ce menu vous permet de configurer les récompenses de niveau, vous pouvez ainsi :
      - Créer / Modifier / Supprimer une [récompense](#récompenses-de-niveau)
      - Réinitialiser les récompenses.

      ![Menu "Récompenses de niveau" via la commande /config](./assets/levels/config_rewards.png)
    ::
    ...
    ::collapse{ label="Annonces" }
      ...
      ![Menu "Annonces" via la commande /config](./assets/levels/config_notifs.png)
    ::
    ...
    ::collapse{ label="Limitations & Boosts" }
      ...
      - à quels rôles appliquer un multiplicateur (x1.5, x2, x2.5, x3),
      - à quels salons appliquer un multiplicateur (x1.5, x2, x2.5, x3).

      ![Menu "Limitations & Boosts" via la commande /config](./assets/levels/config_limits_boosts.png)
    ::
    ...
    ::collapse{ label="Personnalisation" }
      ...
      ![Menu "Personnalisation" via la commande /config](./assets/levels/config_customization.png)
    ::
  ::
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Menu des récompenses depuis le Panel DraftBot](./assets/levels/panel_rewards.png)

    1. **Créer une récompense** : cette option permet de créer et assigner des récompenses aux passages de niveaux de votre choix. *(Apprenez à [créer une récompense](#créer-une-récompense) !)*
    ...
    ::collapse{ label="Voir comment configurer les annonces :" }
      ...
      ![Configuration des annonces depuis le Panel](./assets/levels/panel_rewards_notifs.png)
    ::

    ::hint{ type="success" }
      Si vous avez déjà configuré des [articles de boutique](/docs/engagement/economie#créer-un-article-de-boutique), ils apparaîtront en bas de l'écran, et vous pourrez les modifier / supprimer.
    ::
  ::
  ...
  ::tab{ label="Via la commande /config" }
    ...
    - Créer / Modifier / Supprimer une [récompense](#créer-une-récompense)
    - Réinitialiser les récompenses

    ![Menu "Récompenses" via la commande /config](./assets/levels/config_rewards.png)
  ::
::

### Créer une récompense

DraftBot vous permet de récompenser l'activité de vos membres à travers des items de types variés :

![Menu de création de récompenses, depuis le panel](./assets/levels/panel_create_reward.png)
...
::tabs
  ...
  ::tab{ label="Argent" }
    Si le [système d'économie](/docs/engagement/economie) est activé, vous pouvez récompenser vos membres avec de l'argent virtuel du serveur.
  ...
  ::tab{ label="Objet Inventaire" }
    Récompensez vos membres avec des [objets d'inventaire](/docs/engagement/inventaire) personnalisés et fictifs.

    Choisissez :
    - Le niveau requis
    - L'objet à accorder (parmi ceux existants ou en créer un nouveau)
...
::hint{ type="info" }
  Vous pouvez mettre en place jusqu'à 10 récompenses de niveaux. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> n'ont aucune limite !
::
...
![Bouton d'importation des niveaux](./assets/levels/config_import.png)
