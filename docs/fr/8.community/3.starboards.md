...
::hint{ type="info" }
  Les **sondages**, les **autocollants** et les **messages transférés** apparaissent dans les messages mis en avant, même si l'auteur ne fait plus partie du serveur.
::

## Configuration du système

::hint{ type="info" }
  Vous pouvez créer **un starboard**. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent en créer jusqu'à **3**.
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Descendez dans la liste des systèmes et vous trouverez la section dédiée aux starboards.

    ![La configuration via le panel](./assets/starboards/panel_view.png)
    ...
    ![Menu de création](./assets/starboards/panel_create.png)
    ...
    ::hint{ type="info" }
      ...
      ![La configuration via le panel](./assets/starboards/panel_hub_line.png)
    ::
  ::
  ...
  ::tab{ label="Via la commande /config" }
    ...
    Pour configurer le starboard directement sur Discord, utilisez la commande \</config>, puis sélectionnez l'onglet "Starboard". Après avoir cliqué sur le bouton "Créer", laissez-vous guider par les questions afin de configurer le starboard à l'image de votre serveur.

    Vous pouvez notamment définir le salon du starboard, ainsi que son nom.
    La personnalisation de l'émoji est réservée aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>.
...
- **Émoji** : Permet de modifier l'émoji par défaut du starboard (<:icon_premium:1096140508625125417>)

- **Réactions minimums** : Permet de définir un nombre minimum de réactions (entre 1 et 25).

- **Retirer les réactions de l'auteur** : Permet de retirer la réaction de l'émoji du starboard laissée par l'auteur sur ses propres messages.
...
- Permet de modifier le contenu du message, et de choisir s'il est envoyé dans un **embed** ou non.

- Si l'embed est activé, il sera possible de changer sa couleur. (<:icon_premium:1096140508625125417>)
...
::tabs
  ::tab{ label="Message via un Embed" }
    ![Message via un Embed](./assets/starboards/bot_message.png)
  ::

  ::tab{ label="Message sans Embed" }
    ![Message sans Embed](./assets/starboards/bot_message_2.png)
  ::
::
