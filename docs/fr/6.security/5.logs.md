...
Les logs sont un historique des actions réalisées sur votre serveur. Ils vous permettent de retrouver une action qui vous intrigue ou bien de voir qui a fait quelle action.
...
> ***DraftBot** gère ses propres webhooks et l'envoi des messages avec ces derniers. Si vous supprimez son webhook de logs, il le recréera automatiquement lors de l'envoi du prochain log.*
...
::tabs
  ...
  ::tab{ label="Via la commande /config" }
    ...
    **DraftBot** vous affichera alors la configuration actuelle :
    ...
    - Salon d'envoi
    - Couleur
    - [Destinations](#destinations) personnalisées, si vous en avez configuré
    ...
    ::hint{ type="warning" }
      Le bouton "Réinitialiser" est irréversible : impossible d'annuler l'action ou de récupérer votre configuration précédente.
    ::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    - Pour changer la photo de profil du log, lors de l'envoi : cliquez sur le logo de **DraftBot**. (<:icon_premium:1096140508625125417>)
    - Pour changer la couleur du log, lors de l'envoi : cliquez sur la palette de peinture et sélectionnez la couleur voulue. (<:icon_premium:1096140508625125417>)
  ...
  ::tab{ label="Via la commande /config" }
    ...
    - Pour choisir où les logs sont envoyés : cliquez sur le bouton `Destination` (voir la section [Destinations](#destinations)).
...
Nous vous conseillons de rendre vos logs accessibles uniquement aux modérateurs.

#### Configurations
- **Configuration** : Configuration des systèmes DraftBot de votre serveur (panel compris).
- **Arrivées et départs** : Logs d’arrivée ou de départ des membres de votre serveur.
...
- **Transactions** : Transactions dans le système d’économie ou échanges d'items entre vos membres.
- **Évènements** : Messages lors d’évènements ou jeux communautaires sur votre serveur.
- **Vocal** : Messages lorsqu’un membre quitte, rejoint ou se déplace dans un salon vocal, ainsi que les actions effectuées dans les **salons vocaux temporaires** depuis l'embed de réglages : changements de visibilité, gestion des permissions, mise à jour des listes blanche/noire, transferts et réquisitions, renommages, modifications de la limite d'utilisateurs, et purges.
- **Conférences** : Messages lors du lancement, de la modification ou de la fin d'une conférence.
...
- **Messages** : Modification ou suppression de messages sur votre serveur. (<:icon_premium:1096140508625125417>)
- **Boosts** : Boosts reçus et perdus du serveur. (<:icon_premium:1096140508625125417>)
...
Par défaut, les logs d'un module sont envoyés dans le **salon du serveur** que vous lui avez configuré. Le bouton `Destination` permet d'y ajouter deux sorties, et de désactiver le salon du serveur pour n'envoyer les logs qu'à l'extérieur :

- **Autre serveur** : un **webhook Discord**. Le salon de destination pouvant appartenir à un autre serveur, les mentions de rôles et de salons y sont remplacées par leur nom.
- **API Externe** : l'URL de votre choix, appelée en **HTTP POST** au format JSON, avec jusqu'à **3 headers** pour transmettre par exemple une clé d'authentification.

Il doit toujours rester au moins une destination active : le salon du serveur ne peut être désactivé qu'une fois l'une des deux autres configurée **et** activée.

::hint{ type="info" }
  Ces deux destinations font sortir les logs de votre serveur. Elles sont réservées aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>, et seul le **propriétaire du serveur** peut les configurer ou désactiver le salon du serveur.
::
