...
- L'option **blocage des messages** permet de bloquer les messages avant leur envoi, grâce au système d'AutoMod Discord <:automod:1369109533284106260> tout en comptabilisant l'infraction dans l'historique du membre. L'AutoMod analysant aussi le **statut des salons vocaux**, une infraction qui y est détectée est signalée comme telle dans sa raison : *"Détecté dans le statut d'un salon vocal."*
...
::hint{ type="info" }
  Cette option est une fonctionnalité réservée aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>.
::
...
::hint{ type="info" }
  Une mention de rôle compte pour **3** mentions utilisateurs.
::
...
::hint{ type="warning" }
  Si le blocage des mentions via **Discord** est activé, alors les mentions seront comptabilisées par **message**, indépendamment du système proposé par DraftBot. De plus, les mentions issues des **réponses** ne seront **pas comptabilisées**.
::
...
::hint{ type="info" }
  Vous pouvez configurer jusqu'à 10 sanctions automatiques. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent en avoir 50.
::

::hint{ type="info" }
  Lorsque vous choisissez comme sanction automatique un **bannissement** *(permanent ou temporaire)*, vous pouvez aussi choisir la "**Suppression des messages**". Cela permettra de choisir, ou non, si vous souhaitez supprimer les messages de l'utilisateur sur une certaine période *(Dernière heure, dernières 6 heures, dernières 12 heures, dernières 24 heures, derniers 3 jours ou derniers 7 jours)*.
::
