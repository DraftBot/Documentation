---
slug: /jeux-et-evenements/route-infini
...
## Qu'est-ce que la Route de l'Infini ?

La Route de l'Infini est un système de comptage collaboratif : vos membres envoient des nombres dans un salon dédié, en respectant une suite numérique continue.

Chaque membre doit envoyer le nombre suivant par rapport au précédent. Si une erreur est détectée (mauvais nombre, messages consécutifs, format invalide), **DraftBot** applique automatiquement les règles configurées par les administrateurs.

La gestion du comptage est entièrement assurée par **DraftBot**, qui analyse chaque message envoyé dans le salon configuré.

> ***DraftBot** surveille en continu le salon. À chaque message, il vérifie le nombre, l'auteur, le format et les règles actives avant de valider ou refuser le comptage.*
...
- **Alternance obligatoire** : Vous ne pouvez pas envoyer deux nombres consécutifs. Un autre membre doit compter entre vos deux messages, sauf si votre serveur autorise les [messages consécutifs](#messages-consécutifs)
...
![Exemple](./assets/infinite-road/message_counting.png)
...
![Exemple avec discussion](./assets/infinite-road/message_counting_discussion.png)
...
![Interface de la Route de l'Infini](./assets/infinite-road/bot_infiniteroad_command.png)
...
| **Argent** | De l'argent du [système d'économie](/docs/engagement/economie) | 5000 pièces au nombre 100 |
| **Expérience** | Points d'XP du [système de niveaux](/docs/engagement/niveaux) | 1000 XP au nombre 250 |
| **Objet d'inventaire** | Un ou plusieurs [objets d'inventaire](/docs/engagement/inventaire) | 3x "Badge d'honneur" au nombre 500 |
| **Récompense personnalisée** | Une récompense que vous remettez manuellement | Code promo, mention spéciale, etc. |

::hint{ type="info" }
  Lorsqu'une récompense est basée sur [l'argent du système d'économie](/docs/engagement/economie) du serveur, celle-ci peut provenir :
  - **De la création** : L'argent est créé pour l'occasion
  - **De la banque du serveur** : L'argent provient de l'argent collecté via les erreurs (voir [Actions d'erreurs](#actions-derreurs))
::

::hint{ type="info" }
  Vous pouvez créer jusqu'à **3 récompenses**. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent créer jusqu'à **10 récompenses** !
::

### Limiter les gains répétés

Sur une récompense dont l'objectif est un **multiple**, vous pouvez éviter qu'un même membre accapare tous les paliers :

| Option | Description |
|--------|-------------|
| **Pas deux fois de suite** | Si le même membre déclenche le palier deux fois d'affilée, la récompense ne lui est pas redonnée. Il pourra de nouveau la gagner au palier suivant |
| **Limiter le nombre de gains** | Un membre ne peut remporter cette récompense qu'un nombre de fois défini, de **1 à 1000** |

Dans les deux cas, le membre concerné est prévenu par un message dans le [salon d'annonces des récompenses](#annonces-de-récompenses), s'il est configuré.

::hint{ type="info" }
  Dans la commande \</config>, ces deux options s'appellent respectivement **"Gains consécutifs"** et **"Limite de gains"**.
::

::hint{ type="info" }
  Ces options sont réservées aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>, et ne sont proposées **que sur les récompenses de type "Multiple"**.
::

### Récompenses mystères

Pour garder un effet de surprise, vous pouvez masquer une partie d'une récompense dans la commande \</route-infini> :

| Élément masqué | Effet |
|----------------|-------|
| **Masquer la récompense** | Le contenu de la récompense est remplacé par "🎁 Récompense mystère", mais le palier reste visible |
| **Masquer le nombre cible** | Le palier devient "À un certain nombre" ou "À intervalles réguliers", mais la récompense reste visible |

Vous pouvez masquer l'un des deux éléments, ou les deux.

![Affichage d'une récompense mystère via la commande /route-infini](./assets/infinite-road/bot_mystery_gift.png)

::hint{ type="info" }
  Contrairement aux options ci-dessus, le masquage est disponible sur **tous les serveurs**, et pour les deux types d'objectif (nombre spécifique ou multiple).
::
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    Lors de la création ou de la modification d'une récompense, la partie **"Masquer dans /route-infini"** apparaît dès que vous avez choisi le type de récompense. Activez les éléments que vous souhaitez cacher à vos membres.

    ![Options de masquage dans la configuration d'une récompense sur le panel](./assets/infinite-road/panel_hide_elements.png)

    ::hint{ type="warning" }
      N'oubliez pas de valider avec le bouton "Créer" (ou "Modifier"), puis d'enregistrer vos modifications avec le bouton "Enregistrer" situé en bas de page !
    ::
  ::

  ::tab{ label="Via la commande /route-infini" }
    Si vous êtes animateur de la Route de l'Infini ou administrateur, la commande \</route-infini> affiche un bouton **"Gérer les récompenses"**. Il vous permet de :

    - Créer, modifier ou supprimer une récompense
    - Réinitialiser **toutes** les récompenses

    En modifiant une récompense, le bouton **"Éléments masqués"** vous permet de sélectionner ce que vous souhaitez cacher.

    ![Bouton "Éléments masqués" lors de la modification d'une récompense](./assets/infinite-road/config_hide_elements.png)
  ::
::
...
::hint{ type="info" }
  Vous pouvez créer jusqu'à **3 déclencheurs** gratuitement. Les serveurs [premium](/premium) <:icon_premium:1096140508625125417> peuvent créer jusqu'à **10 déclencheurs** !
::
...
Le système détecte automatiquement quatre types d'erreurs. Vous choisissez ensuite pour **lesquels** appliquer une action supplémentaire (réinitialisation, retrait d'argent, rôle temporaire).
...
| **Messages consécutifs** | Le membre envoie plus de nombres d'affilée que ne l'autorise l'option [Messages consécutifs](#messages-consécutifs), sans qu'un autre membre compte entre-temps |
| **Commande envoyée** | Le membre exécute une commande dans le salon de comptage |
...
Le message envoyé lors de la réinitialisation est personnalisable (**1000 caractères maximum**) avec les variables suivantes :

| Variable | Description | Exemple |
|----------|-------------|---------|
| `{infiniteroad.number}` | Le prochain nombre attendu après la réinitialisation | 1 |
| `{infiniteroad.error_type}` | Le type d'erreur ayant provoqué la réinitialisation | un **nombre incorrect** |

*En plus des [autres variables](/docs/autres/variables) déjà disponibles globalement !*

**Message par défaut** : `❌ {user} a ruiné la Route de l'Infini en envoyant {infiniteroad.error_type} ! Le compteur a été réinitialisé, le prochain nombre est **{infiniteroad.number}**.`

::tabs
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/infinite-road)

    Activez l'option "Réinitialiser la route de l'infini en cas d'erreur" : la partie "Message de réinitialisation personnalisé" apparaît alors juste en dessous.

    ![Configuration du message de réinitialisation sur le panel](./assets/infinite-road/panel_reset_message.png)
  ::

  ::tab{ label="Via la commande /config" }
    Rendez-vous dans la commande \</config>, puis dans l'onglet "Route de l'Infini". Appuyez ensuite sur le bouton "Erreurs", puis sur "Message de réinitialisation".

    ![Réponse de DraftBot après avoir appuyé sur le bouton "Message de réinitialisation"](./assets/infinite-road/bot_reset_message.png)
  ::
::

::hint{ type="info" }
  Le markdown est autorisé dans ce message.
::

### Retrait d'argent

Si le [système d'économie](/docs/engagement/economie) est activé, vous pouvez retirer de l'argent aux membres qui font certaines erreurs (désactivé par défaut). Deux modes sont disponibles :
...
- **Embed** : Affichage dans un embed Discord avec ou sans couleur et personnalisable avec des variables
...
::hint{ type="info" }
  La personnalisation du format est réservée aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>.
::

### Couleur du système

La couleur du système s'applique aussi bien aux messages renvoyés au format **Embed** qu'à l'affichage de la commande \</route-infini>. Par défaut, il s'agit du **#CD6E57** (orange).

::hint{ type="info" }
  La personnalisation de la couleur est réservée aux serveurs [premium](/premium) <:icon_premium:1096140508625125417>.
::

### Réaction de validation

En mode **Classique**, vous pouvez configurer une réaction qui sera automatiquement ajoutée à chaque message contenant un nombre valide (désactivé par défaut). Utilisez un émoji standard (✅, ⭐) ou un émoji personnalisé de votre serveur.

::hint{ type="warning" }
  Si l'émoji configuré est supprimé ou inaccessible, la réaction sera automatiquement désactivée et vous recevrez une notification.
::
...
Par défaut, un même membre ne peut pas envoyer deux nombres d'affilée : un autre membre doit compter entre ses deux messages. Vous pouvez lever cette règle depuis cette même partie **Discussions** :

- **Autoriser plusieurs messages à la suite** : Permet à un même membre d'envoyer plusieurs nombres d'affilée (désactivé par défaut)
- **Nombre de messages consécutifs autorisés** : Combien de nombres un même membre peut envoyer d'affilée, **de 2 à 20**

Au-delà de cette limite, l'erreur **Messages consécutifs** est déclenchée comme d'habitude.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Mise en place de la Route de l'Infini depuis le Panel](./assets/infinite-road/panel_setup.png)
    ...
    ![Module Route de l'Infini activé sur le Panel](./assets/infinite-road/panel_home.png)
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ![Module Route de l'Infini via la commande /config](./assets/infinite-road/config_home.png)
    ...
    ::collapse{ label="Animateurs" }
      ...
      - Envoyer des messages invalides sans être impactés par la suppression du message ou un malus.
    ::
    ...
    ::collapse{ label="Discussions" }
      ...
      - Autoriser plusieurs messages consécutifs d'un même membre, et définir combien

      ![Menu "Discussions" via la commande /config](./assets/infinite-road/config_discussions.png)
    ::
    ...
    ::collapse{ label="Erreurs" }
      ...
      - **Réinitialisation** : Remettre le compteur à 1 en cas d'erreur
          - Message de réinitialisation personnalisé
      ...
      ![Menu "Erreurs" via la commande /config](./assets/infinite-road/config_errors.png)
    ::
    ...
    ::collapse{ label="Déclencheurs" }
      ...
      ![Menu "Déclencheurs" via la commande /config](./assets/infinite-road/config_triggers.png)
    ::

    ::collapse{ label="Récompenses" }
      Gérez les récompenses de la Route de l'Infini :
      - Créer, modifier ou supprimer des récompenses
      - Masquer le palier ou la récompense dans \</route-infini>
      - Configurer les annonces de récompenses
      - Personnaliser le message d'annonce

      ![Menu "Récompenses" via la commande /config](./assets/infinite-road/config_rewards.png)
    ::
...
- **Message personnalisé** : Personnalisez le message d'annonce avec `{number}`, `{reward}` pour afficher la récompense obtenue, ainsi que les [autres variables](/docs/autres/variables) disponibles globalement
...
5. **Éléments à masquer** dans \</route-infini> (voir [Récompenses mystères](#récompenses-mystères))
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Création de récompense depuis le panel](./assets/infinite-road/panel_rewards.png)
  ::
  ...
  ::tab{ label="Via la commande /config" }
    ...
    ![Création de récompense via /config](./assets/infinite-road/config_rewards.png)
  ::
::
...
![Bouton d'importation Countr](./assets/infinite-road/config_import.png)
