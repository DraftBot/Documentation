...
2. **Restez bienveillants :** Évitez le plus possible les expressions subjectives comme *"simplement"*, *"juste"*, *"logiquement"*. **Gardez en tête que nous avons tous une expérience et un passé différents**. Ces mots n'apportent aucune information utile et peuvent parfois même être réducteurs et blessants, notamment pour les personnes découvrant Discord ou débutant leur utilisation avec **DraftBot**.
...
Lorsqu'un gérant de serveur décide d'activer un système, il se place toujours du point de vue de ses membres. S'il est convaincu, il passera à la mise en place.
...
### Configuration depuis le Panel et /config

Toujours mettre par défaut le choix "**Via le Panel**" plutôt que "**Via la commande /config**" afin de garder une harmonisation entre les pages et que les membres n'aient pas à regarder ça.

::tabs
  ::tab{ label="Depuis le panel" }
    [⫸ Accéder au panel de **DraftBot**](/dashboard/first/niveaux)

    Ceci est pour configurer depuis le panel.
  ::

  ::tab{ label="Via la commande /config" }
    Si vous souhaitez effectuer toute la configuration directement depuis Discord, vous pouvez le faire via la commande /config, puis en vous rendant dans l'onglet "Niveau".
  ::
::
...
Certaines fonctionnalités ont des comportements spécifiques dans des cas particuliers (edge cases). Documentez-les clairement pour éviter la confusion :
...
Toutes les pages de la documentation *(hormis certaines pages)* doivent définir une icône via `navigation.icon`.

Deux bibliothèques d'icônes sont utilisées :

- **Les [`twemoji`](https://icones.js.org/collection/twemoji) :** **Uniquement** pour les **pages de fonctionnalités** *(Accueil des membres, Engagement, Jeux & événements, Communauté, Sécurité, Utilitaires)* et les pages **d'accueil/installation**.
- **Les [`material-symbols`](https://icones.js.org/collection/material-symbols?s=filter&variant=Outline) :** Pour **toutes les pages** de la documentation.
...
La documentation existe en français et en anglais. Le français fait autorité :
toute page est d'abord écrite en français, puis traduite.

### Où placer un fichier

Les pages françaises vivent sous `docs/fr/`, les anglaises sous `docs/en/`.
**Les noms de fichiers et de dossiers sont en anglais et identiques dans les
deux langues** — c'est ce qui permet d'apparier une page et sa traduction.
Une page française s'appelle donc `docs/fr/3.engagement/0.levels.md`, et non
`0.niveaux.md`.

Les préfixes numériques déterminent l'ordre d'affichage dans le menu.

### Déclarer l'adresse d'une page

L'adresse d'une page ne vient plus de son nom de fichier mais de son
frontmatter :
...
Le `slug` est le chemin complet, sans le préfixe `/docs/fr`. Il doit
commencer par le `slug` du fichier `_dir.yml` de son dossier, privé de son
suffixe `/_dir`.

Les fichiers `_dir.yml` en portent un aussi, terminé par `/_dir` :
...
Les images d'une page vivent dans le dossier `assets/` de sa catégorie, dans
un sous-dossier au nom de la page :

```
docs/fr/3.engagement/
├─ assets/levels/panel.png
└─ 0.levels.md              ![Légende](./assets/levels/panel.png)
```

### Vérifier son travail

```bash
node scripts/check-translations.mjs   # slugs, doublons et traductions à jour
node scripts/check-links.mjs          # images et liens internes introuvables
```

### Traductions

Vous n'avez pas à traduire vous-même. Quand une page française change, une
pull request de traduction est ouverte automatiquement — elle a besoin d'être
relue, pas écrite. En attendant sa relecture, la documentation anglaise
affiche la version française à jour plutôt qu'une traduction dépassée.

### Exclure une page de la traduction

Une page qui n'a pas vocation à exister en anglais porte `translate: false`
...
Elle sort alors de la liste des pages à traduire. C'est le cas du changelog,
qui reste en français.

Cette clé ne se propage pas : la poser dans un `_dir.yml` n'exclut que ce
fichier, pas les pages du dossier. Chaque page à exclure doit la porter.
