...
::hint{ type="info" }
  Depuis la commande \</premium codes>, un bouton **"Réclamer un code"** permet également d'ajouter un code premium directement à votre inventaire, sans passer par le panel. Vous pourrez ensuite l'activer quand vous le souhaitez.
::

::hint{ type="info" }
  Depuis le [panel web](/dashboard/user/premium), le bouton **"Activer le renouvellement"** transforme votre code en abonnement. Vous choisissez une [formule](/premium) (mensuelle, 6 mois ou annuelle) : rien n'est prélevé aujourd'hui, votre code va jusqu'à son terme et l'abonnement prend le relais à cette date. Vous pouvez le résilier à tout moment. Si la formule choisie couvre moins de serveurs que votre code actuel, le panel vous prévient avant de valider.
::
...
| \</premium codes>   | Afficher son inventaire de code premium, et réclamer un nouveau code via le bouton "Réclamer un code".
...
![Aperçu de la fenêtre lors de l'activation de votre code](./assets/premium/apercu_popup_accumulation.png)
