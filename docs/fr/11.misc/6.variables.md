...
to: /docs/communaute/commandes-personnalisees#les-arguments
target: _blank
color: '#FAEF90'
---
...
| Variable | Description | Exemple |
|----------|-------------|---------|
| `{user}` ou `{users}` | Membre(s) fêtant leur anniversaire | @John, @Jane |
...
### Réinitialisation de la route de l'infini

| Variable | Description | Exemple |
|----------|-------------|---------|
| `{infiniteroad.number}` | Prochain nombre attendu après la réinitialisation | 1 |
| `{infiniteroad.error_type}` | Type d'erreur ayant provoqué la réinitialisation | un **nombre incorrect** |
...
### Messages de sanctions en MP

| Variable | Description | Exemple |
|----------|-------------|---------|
| `{sanction.moderator}` | Nom du modérateur à l'origine de la sanction | DraftBot |
| `{sanction.end}` | Date de fin de la sanction temporaire (si applicable) | dans 3 jours |
| `{sanction.duration}` | Durée de la sanction temporaire (si applicable) | 5 jours |
| `{sanction.reason}` | Raison de la sanction | Publicité sur le serveur |
...
| `{stream.start_at}` ou `{stream.started_at}` | Date de début | il y a 2 heures |
