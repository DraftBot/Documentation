...
title: Tickets
description: Offrez à vos membres la possibilité de créer des tickets afin d'avoir une discussion privée avec l'équipe de votre serveur.
...
![Menu de configuration](./assets/tickets/config_menu.png)
...
::hint{ type="info" }
  La variable `raison`, bien que facultative, peut être rendue obligatoire par les gérants du serveur. Il est conseillé de toujours la remplir.
::
...
- \</ticketmod ouvrir> ➜ Permet d'ouvrir un ticket pour un membre.
- \</ticketmod ajouter> ➜ Permet d'autoriser l'accès au ticket à un membre tiers.
- \</ticketmod retirer> ➜ Permet de retirer l'accès au ticket à un membre.

### Salon de réception

Le salon de réception des tickets permet de recevoir les demandes de tickets des membres, qu'elles soient effectuées via la commande \</ticket> ou bien via un **message d'ouverture**.
...
::tabs
  ::tab{ label="Depuis le panel" }
    ...
    ![Menu de configuration](./assets/tickets/panel_config.png)
  ...
  ::tab{ label="Via la commande /config" }
    Pour configurer le système de tickets, vous pouvez utiliser la commande \</config> puis cliquer sur la section "Tickets".
    ...
    ![Menu de configuration de départ](./assets/tickets/menu_config.png)
    ...
    > **Supprimer le système** ➜ Permet de réinitialiser le système et de revenir au bouton "Configurer le système".
    ...
    > **Mentionner les modérateurs** ➜ Activer/désactiver la mention automatique des rôles indiqués comme rôles modérateurs à chaque nouveau ticket ouvert. Si cette option est activée, tous les rôles modérateurs seront mentionnés. Il n'est actuellement pas possible de choisir les rôles spécifiques à mentionner parmi la liste des rôles modérateurs.

    > **Suppression admin** ➜ Les tickets sont directement supprimés si un administrateur de serveur ferme un ticket.

    > **Format du nom des salons** ➜ Permet de choisir comment sont nommés les salons de tickets. Le format par défaut est `{user.globalname}`. Vous pouvez y employer les [variables](/docs/autres/variables), dont `{number}`, qui numérote le ticket.
  ::
::

### Message d'ouverture

![Configuration du message d'ouverture](./assets/tickets/config_reason.png)
...
::hint{ type="info" }
  Vous pouvez vous servir d'un message préalablement créé depuis l'[Embed Creator](/dashboard/first/embeds) sur le panel en cliquant sur "Message existant" sur la question posée après sélection du type de message d'ouverture pour personnaliser à votre guise l'embed de présentation ou via la commande \</envoyer>.
::
