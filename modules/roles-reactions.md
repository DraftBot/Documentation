...
Une fois cela fait, plusieurs choix s'offrent à vous :

* ***Nouveau message*** ➜ **DraftBot** va créer un embed au titre personnalisé.
* ***Message existant de DraftBot*** ➜ Le bot va récupérer un message de **DraftBot** ayant déjà été envoyé.

{% hint style="warning" %}
Pour créer un rôle-réaction à partir d'un message déjà envoyé, il vous faut récupérer son <mark style="color:blue;">[identifiant](../autres/recuperer-un-identifiant.md#identifiant-dun-message)</mark>.
{% endhint %}

{% hint style="info" %}
Cependant, si vous souhaitez créer un nouveau message pour votre rôle-réaction voici les différentes possibilités :
...
L'intégralité de la création est ensuite guidée par **DraftBot**, vous devrez alors indiquer le rôle devant être ajouté lors d'une interaction avec le message et le format du rôle-réaction.

{% hint style="info" %}
Il est possible de donner un rôle temporairement en choisissant le format bouton.
{% endhint %}

{% hint style="success" %}
Félicitations ! Le rôle-réaction est maintenant créé. Vous allez pouvoir y ajouter d'autres rôles et continuer à le personnaliser.
{% endhint %}
...
Voici les différentes possibilitées pour définir le message sur lequel les membres devront intéragir pour recevoir un rôle :
...
Pour ajouter un nouveau rôle à votre rôle-réaction, vous n'avez qu'à cliquer sur "<mark style="color:blue;">Ajouter</mark>". **DraftBot** vous demandera ensuite le rôle à ajouter ainsi que le format.
...
Si vous souhaitez changer le rôle que le membre reçoit en interagissant avec un bouton, une réaction ou un rôle à choix dans un sélecteur, cliquez sur "<mark style="color:blue;">Modifier</mark>". **DraftBot** vous invitera à sélectionner le rôle à modifier et celui par lequel vous souhaitez le remplacer.
...
À droite de la page, se trouve la liste contenant l'ensemble les rôles-réactions existants sur le serveur. Pour en modifier un, cliquer sur "Modifier".

Vous pouvez par la suite modifier les rôles-réactions de différentes façons :
